...
void CONTROL_SwitchStateToDisabled(uint16_t DisableReason, uint16_t DisableReasonEx)
{
	InfoPrint(IP_Err, "Switched to disabled");
...
void CONTROL_PisoScopeInit()
{
	PICO_STATUS status = LOGIC_PisoScopeInit();
	...
	if (CONTROL_State == DS_InProcess)
	{
		if (SAMPLING_Finished())
		{
			...
			else
			{
				...
				uint16_t EPSize = DataTable[REG_EP_SIZE_REDEFINE] ? DataTable[REG_EP_SIZE_REDEFINE] : VALUES_READx_SIZE;
				MEMBUF_Values1_Counter = LOGIC_GetIData(MEMBUF_Values1, EPSize, CalcOK, DataTable[REG_MEASURE_MODE] == MODE_QRR, Index0, Index0V, forced_sector, &SampleTimeStep);
				MEMBUF_Values2_Counter = LOGIC_GetVData(MEMBUF_Values2, EPSize, CalcOK, DataTable[REG_MEASURE_MODE] == MODE_QRR, Index0, Index0V, forced_sector, NULL);
		...
		case ACT_START_TEST:
			{
				...
				{
					PICO_STATUS status = LOGIC_PisoScopeActivate(DataTable[REG_CURRENT_AMPL], (float)DataTable[REG_SHUNT_RES_N] / DataTable[REG_SHUNT_RES_D], (DataTable[REG_ENABLE_VOLTAGE_SMPL] == 0) ? false : true);
					if (status == PICO_OK)
					{
						CONTROL_FillWPPartDefault();
						DEVPROFILE_ResetEPs();
